...
$questoes_por_assunto = $questoes_obj->carregar($_SESSION['assunto_cod']);
////////////////////////////////////////////////////
$materia_cod = isset($_POST['materia_cod'])?$_POST['materia_cod']:"";
$prova_cod = isset($_SESSION['prova_cod'])?$_SESSION['prova_cod']:"";
...
$cod_ed = isset($_POST['cod_ed'])?$_POST['cod_ed']:"";
$imagem_ed = isset($_POST['imagem_ed'])?$_POST['imagem_ed']:"";
...
if (!empty($imagem_ed)) {
  $_SESSION['imagem_ed'] = $imagem_ed;
}        
if (!empty($cod_ed)) {
  $_SESSION['cod_ed'] = $cod_ed;
...
if(!empty($cod_apagar)){
  $questoes_obj->deletar($cod_apagar);
}
    ...
    <ul class="navbar-nav bg-gradient-primary sidebar sidebar-dark accordion" id="accordionSidebar">
              ...
              <center>
                ...
                <div class="d-sm-flex align-items-center justify-content-between mb-4">                    
                  <table class='table '>
                    <thead>
                      <tr>
                        <th width='75%'; scope='col'>Corpo</th>
                        <th scope='col'>Editar</th>
                        <th scope='col'>Apagar</th>
                      </tr>
                    </thead>
                    <tbody> 
                      <?php foreach ($questoes_por_assunto as $x) {
                        echo("<tr> <form method='POST' id='form_p'>");
                        echo("<td scope='row'>".$x['corpo']."</td>");
                        echo("<td><button type='button' onclick='A(");
                        echo($x['cod'].','.'"'.$x['imagem'].'"');
                        echo(")' class='btn btn-success'><i class='fas fa-edit'></i></button></td>");
      ...
      <div class="modal fade" id="CadastrarModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" >
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            <div class="modal-header">
              <h5 class="modal-title" id="exampleModalLabel">Selecione o Assunto <br></h5>
              <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">×</span>
              </button>
            </div>
            <div class="modal-body">
              <form method="POST">
                <h5>Não é possível editar ou apagar questões cadastradas, tenha total certeza antes de salvar.</h5>
              </div>
              <div class="modal-footer">
                <button class="btn btn-secondary" type="button" data-dismiss="modal">Cancelar</button>
            ...
            <form method="POST" id="form_ed">
              <input type="hidden" name="cod_ed" id="cod_ed">
              <input type="hidden" name="imagem_ed" id="imagem_ed">
            </form>
          </div>
        </div>
      </div>
      ...
      <script type="text/javascript">
        ...
        function A(cod,imagem){
          var cod = cod;
          var imagem = imagem;
          $('#cod_ed').val(cod);                                 
          $('#imagem_ed').val(imagem);
          $('#form_ed').submit();
        }
        function P(cod) {
          var cod = cod;
